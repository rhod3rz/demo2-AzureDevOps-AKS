...
# Create azure firewall to restrict egress traffic.
# Note, the firewall & pip must be created in the same resource group that contains the 'AzureFirewallSubnet', e.g. rg-core-01.
module "az_firewall" {
  source                          = "../../mod/az_firewall"                                 /* The path to the module */
  pip_afw_primary                 = "${module.az_config.item_name.pip_afw_primary}"         /* The pip name */
  afw_primary                     = "${module.az_config.item_name.afw_primary}"             /* The afw name */
  rt_name                         = "rt-aks-egress-via-afw"                                 /* The route table name */
  rtr_name_virtual_appliance      = "rn-virtual-appliance"                                  /* The route table rule name */
  rtr_name_internet               = "rn-internet"                                           /* The route table rule name */
}

# Create firewall rules
module "az_firewall_rules" {
  source                          = "../../mod/az_firewall_rules"                           /* The path to the module */
  depends_on                      = [module.az_firewall]                                    /* Wait for dependencies */
  afw_primary                     = "${module.az_config.item_name.afw_primary}"             /* The afw name */
}
...
module "az_helm" {
  ...
  depends_on                      = [module.az_aks,module.az_firewall_rules]                /* Wait for dependencies */
